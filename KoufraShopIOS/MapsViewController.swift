...
class MapsViewController: UIViewController,UISearchBarDelegate, MKMapViewDelegate{
    //variables
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        let txt="Calle Venustiano Carranza 2 De la Cruz 58880 Tarímbaro, MICH Mexico"
        
        //crear una variable segura
        if let direccion = buscadorSB.text{
            geocoder.geocodeAddressString(txt) { (places: [CLPlacemark]?, error: Error?) in
                //crear el destino
    ...
    @IBAction func btnRuta(_ sender: UIButton) {
        let geocoder = CLGeocoder()
        let txt="Calle Venustiano Carranza 2 De la Cruz 58880 Tarímbaro, MICH Mexico"
        geocoder.geocodeAddressString(txt) { (places: [CLPlacemark]?, error: Error?) in
            //crear el destino
